...
The physical mechanism behind this asymmetric isolation lies in the extreme \emph{acoustic impedance mismatch} between the ambient air and the solid drywall. Airborne acoustic waves (such as the jammer $v(t)$) possess very low acoustic impedance. When these waves strike a high-density, high-stiffness structural barrier from inside the receiving room, more than 99.9\% of the acoustic energy is reflected back into the room rather than absorbed as structural vibration. As a result, the macroscopic drywall acts as a mechanical low-pass filter with an exceptionally high activation threshold. It resonates measurably to the target source $s(t)$ arriving from the adjacent space, but remains virtually inert to the airborne jammer $v(t)$ bouncing off its surface. This unique vibro-acoustic property guarantees that the LDV signal $x_{\text{ldv}}(t)$ operates as a unilaterally isolated channel.
...
This cross-modal cross-correlation yields two crucial physical invariants: jammer decorrelation and spectral cancellation. To understand the latter, consider the idealized noise-free cross-spectrum $G_{Vm}(f) = X_v(f)X_m^*(f)$. Substituting the signal models from Eq.~\ref{eq:mics} and \ref{eq:ldv} (ignoring the jammer $v(t)$ which vanishes in expectation), we obtain:
%
\begin{align}
  G_{Vm}(f) &= \left[a_v^s (j2\pi f) S(f) H_{\text{surf}}(f) e^{-j2\pi f \tau_v^s}\right] \nonumber \\
            &\quad \times \left[a_m^s S(f) H_{\text{wall}}(f) e^{-j2\pi f \tau_m^s}\right]^* \nonumber \\
            &= \Phi_{SS}(f) H_{\text{surf}}(f) H_{\text{wall}}^*(f) (-j2\pi f) e^{j2\pi f (\tau_m^s - \tau_v^s)}
\end{align}
%
where $\Phi_{SS}(f) = a_v^s a_m^s |S(f)|^2$ is the source power spectral density. The phase of this cross-spectrum contains the desired spatial delay $\Delta\tau_{mv} = \tau_m^s - \tau_v^s$, but it is severely corrupted by the complex phase of $H_{\text{wall}}(f)$. 

However, by applying the PHAT weighting $\Psi(f) = |G_{Vm}(f)|^{-1}$, we normalize the magnitude, neutralizing the variable and material-specific attenuation $|H_{\text{wall}}(f)|$. Crucially, the phase corruption introduced by the barrier, $\angle H_{\text{wall}}(f)$, is inherently deterministic and structurally identical for both the LDV-MicL and LDV-MicR signal pairs, assuming the target source is in the far-field relative to the barrier's sub-wavelength local thickness variations. When the network processes the paired cross-modal GCC-PHAT spatial features, the shared wall transfer function $H_{\text{wall}}(f)$ mathematically cancels out in the differential delay domain ($\Delta\tau = \tau_{VR} - \tau_{VL}$). The resulting concatenated feature vector acts as a pristine, material-agnostic representation of the spatial geometry, perfectly isolating the geometric Time-Difference-of-Arrival and guaranteeing generalization across diverse barrier materials without requiring the network to memorize specific acoustic impedances.

\subsection{DNN Architecture and Loss Formulation}
\label{sec:dnn_arch}

The extracted cross-modal features $R_{VL}(\tau)$ and $R_{VR}(\tau)$ are concatenated and fed into a lightweight multi-layer perceptron (MLP), as detailed in the overall pipeline of Fig.~\ref{fig:arch}. Because the GCC-PHAT transformation has already resolved the complex frequency-domain spatial mixtures into an explicit time-difference map, the DNN acts primarily as a non-linear spatial regressor mapping time delays to physical angles. 

Crucially, while classical Physics-Informed Neural Networks (PINNs) enforce physical constraints via complex auxiliary loss functions—which often suffer from severe convergence instability—our architecture achieves physical compliance directly through its input domain. By mathematically embedding the acoustic transmission physics into the cross-modal features prior to the network, we avoid backpropagating through ill-posed barrier transfer functions and can utilize a simple, stable Mean Squared Error (MSE) loss against the ground truth angle $\theta_{\text{true}}$. 

The architecture comprises three fully connected layers of sizes 128, 64, and 1, interspersed with ReLU activations. The final layer outputs a continuous scalar representing the estimated DoA angle $\hat{\theta}$. This shallow structure uniquely sidesteps the computational overhead of PDE-based loss calculations, ensuring true real-time applicability while remaining strictly bounded by the physical propagation geometry.
...
To evaluate robustness against interference, a second "jammer" speaker is placed inside the receiving room, 1.0\,m from the microphones. The jammer broadcasts continuous babble noise, which is completely uncorrelated with the target speech. We define the nominal Signal-to-Jammer Ratio (SJR) based on the unattenuated target power. However, because the target suffers a massive $\approx 35$\,dB transmission loss through the drywall, the \emph{effective} SJR at the microphones is profoundly negative even when the jammer is conversing at a normal volume.
...
\subsection{Ablation Study and Computational Complexity}

To validate the necessity of the GCC-PHAT feature extraction, we conducted an ablation study comparing our PI-DNN against a raw-spectrogram CNN baseline. The CNN baseline was fed the concatenated magnitude and phase STFTs of the LDV and microphones, utilizing 4 convolutional blocks (approx. 1.2M parameters).

Despite its vastly larger capacity, the raw STFT CNN struggled to generalize, yielding a median DoA error of $6.5^\circ$ and a $P_{CW}$ of 12\% under a $-6$\,dB SJR. The network expended its representational capacity attempting to overfit the barrier's specific spectral coloration rather than extracting spatial delays. 

In contrast, our PI-DNN leverages the physics-informed GCC-PHAT transformation to mathematically strip away spectral distortions before training. This allows our architecture to be extremely lightweight, comprising only three fully connected layers with a mere 16,000 parameters. On a standard mobile CPU core (comparable to edge devices used in search-and-rescue robots), the entire fusion and inference pipeline executes in under 2.5 milliseconds per frame. This ablation confirms that injecting physical priors (cross-modal phase transforms) not only dramatically improves tracking robustness against coherent jammers but also compresses the computational footprint by nearly two orders of magnitude, enabling true real-time edge deployment.
