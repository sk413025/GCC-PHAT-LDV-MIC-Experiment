...
Our core design principle is to discard spectral magnitude information prior to the neural network and exclusively preserve phase relationships, acknowledging that laser reflections off unknown surfaces introduce arbitrary magnitude distortions \cite{yamada2023speech}. We compute the Generalized Cross-Correlation with Phase Transform (GCC-PHAT) \cite{knapp1976gcc} not between the two microphones (which is jammed), but between the clean LDV anchor and each individual microphone:
%
\begin{equation}
