#!/usr/bin/env python3
"""
Round 8 lane 1: block6 same-VL temporal persistence.

This lane freezes round6 lane3 `sign_veto_poslift_r0p10` and adds a narrow
block6-only neighbor-supported positive-family retention rule.
"""
...
VARIANTS: list[dict[str, Any]] = [
    ...
    {
        "name": "b6_samevl_hold_s2_vl0p18_dt0p15_rf0p08",
        "support_required": 2,
        "tau_vl_tol_ms": 0.18,
        "tau_vr_tol_ms": 0.18,
        "abs_dt_tol_ms": 0.15,
    ...
    {
        "name": "b6_samevl_hold_s2_vl0p20_dt0p20_rf0p08",
        "support_required": 2,
        "tau_vl_tol_ms": 0.20,
        "tau_vr_tol_ms": 0.20,
        "abs_dt_tol_ms": 0.20,
    ...
    {
        "name": "b6_samevl_hold_s3_vl0p20_dt0p20_rf0p08",
        "support_required": 3,
        "tau_vl_tol_ms": 0.20,
        "tau_vr_tol_ms": 0.20,
        "abs_dt_tol_ms": 0.20,
    ...
    {
        "name": "b6_samevl_hold_s3_vl0p25_dt0p25_rf0p05",
        "support_required": 3,
        "tau_vl_tol_ms": 0.25,
        "tau_vr_tol_ms": 0.25,
        "abs_dt_tol_ms": 0.25,
...
def neighbor_offsets(offset_sec: float) -> list[float]:
    offsets = [clamp_offset(offset_sec - NEIGHBOR_STEP_SEC), float(offset_sec), clamp_offset(offset_sec + NEIGHBOR_STEP_SEC)]
...
def choose_positive_same_vl_family(
    scored_pool: list[dict[str, float]],
    *,
    anchor_tau_vl_ms: float,
    anchor_tau_vr_ms: float,
    anchor_abs_dt_ms: float,
    tau_vl_tol_ms: float,
    tau_vr_tol_ms: float,
    abs_dt_tol_ms: float,
    score_floor: float,
    ...
    for row in scored_pool:
        ...
        if row["score"] < score_floor * top_score:
            continue
        tau_vl_gap = abs(float(row["tau_vl_ms"]) - float(anchor_tau_vl_ms))
        tau_vr_gap = abs(float(row["tau_vr_ms"]) - float(anchor_tau_vr_ms))
        dt_gap = abs(abs(float(row["delta_tau_ms"])) - float(anchor_abs_dt_ms))
        if tau_vl_gap > tau_vl_tol_ms or tau_vr_gap > tau_vr_tol_ms or dt_gap > abs_dt_tol_ms:
            continue
        candidates.append((tau_vl_gap, tau_vr_gap, dt_gap, -float(row["score"]), row))
    if not candidates:
        return None
...
def evaluate_variant_case(
    ...
    *,
    ...
    anchor_by_case: dict[str, dict[str, float]],
) -> dict[str, Any]:
    ...
    if variant["name"] == "control_lane6c_ref" or case.case_id != BLOCK6_CASE_ID:
        return base_row

    anchor = anchor_by_case[case.case_id]
    contexts = {
    ...
    positive_rows = [
        ...
        and abs(row["selected"]["best"]["tau_vl_ms"] - float(anchor["tau_vl_ms"])) <= float(variant["tau_vl_tol_ms"])
        and abs(row["selected"]["best"]["tau_vr_ms"] - float(anchor["tau_vr_ms"])) <= float(variant["tau_vr_tol_ms"])
        and abs(abs(row["selected"]["best"]["delta_tau_ms"]) - abs(float(anchor["delta_tau_ms"]))) <= float(variant["abs_dt_tol_ms"])
    ]
    if len(positive_rows) < int(variant["support_required"]):
        base_row["samevl_support"] = len(positive_rows)
        base_row["samevl_anchor_tau_vl_ms"] = float(anchor["tau_vl_ms"])
        base_row["samevl_anchor_tau_vr_ms"] = float(anchor["tau_vr_ms"])
        base_row["samevl_anchor_abs_dt_ms"] = abs(float(anchor["delta_tau_ms"]))
        return base_row

    current_context = contexts[float(offset_sec)]
    candidate = choose_positive_same_vl_family(
        current_context["scored_pool"],
        anchor_tau_vl_ms=float(anchor["tau_vl_ms"]),
        anchor_tau_vr_ms=float(anchor["tau_vr_ms"]),
        anchor_abs_dt_ms=abs(float(anchor["delta_tau_ms"])),
        tau_vl_tol_ms=float(variant["tau_vl_tol_ms"]),
        tau_vr_tol_ms=float(variant["tau_vr_tol_ms"]),
    ...
    base_row["samevl_promoted"] = True
    base_row["promotion_reason"] = "samevl_temporal_hold"
    base_row["samevl_support"] = len(positive_rows)
    base_row["samevl_anchor_tau_vl_ms"] = float(anchor["tau_vl_ms"])
    base_row["samevl_anchor_tau_vr_ms"] = float(anchor["tau_vr_ms"])
    base_row["samevl_anchor_abs_dt_ms"] = abs(float(anchor["delta_tau_ms"]))
...
def summarize_setting(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "promoted_cases": int(sum(1 for row in rows if row.get("samevl_promoted", False))),
    }
...
def write_report(out_path: Path, payload: dict[str, Any]) -> None:
    lines = [
        "# Round 8 Block6 Same-VL Temporal Persistence",
        "",
        f"- Generated: {payload['generated_at']}",
        f"- Data root: `{payload['data_root']}`",
        "- Frozen reference: round6 lane3 `sign_veto_poslift_r0p10`",
        "",
        "## Summary",
        "",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Round 8 block6 same-VL temporal persistence for 0223.")
    parser.add_argument(
        "--data_root",
        type=Path,
    ...
    out_dir = args.out_dir or (
        Path(__file__).resolve().parent.parent / "results" / f"round8_block6_samevl_temporal_persistence_0223_{timestamp}"
    )
    out_dir.mkdir(parents=True, exist_ok=True)

    full_signals = {case.case_id: lane2.lane1.load_full_case_signals(case, args.data_root) for case in base.CASES}
    anchor_by_case = {
        case.case_id: lane6c.evaluate_variant_case(case, full_signals[case.case_id], window_sec=ANCHOR_WINDOW_SEC, offset_sec=ANCHOR_OFFSET_SEC, variant=LANE6_REF)["selected"]["best"]
