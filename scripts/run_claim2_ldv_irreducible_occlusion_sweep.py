#!/usr/bin/env python
"""
Claim-2 verification driver: mic-local occlusion (spectral distortion) + noise/clipping sweep.

This is a follow-up to the in-band noise+clipping sweep, designed to increase the chance that
baseline MIC–MIC enters a true near-fail regime under mic-local degradations, while LDV remains clean.
"""
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Claim-2 occlusion sweep (existing WAVs only)")
    ...
    occlusion_grid = list(OCCLUSION_GRID)
    ...
    for occ in occlusion_grid:
        occ_name = str(occ["name"])
        for snr_db in snr_grid:
            tag = f"{snr_tag(float(snr_db))}_occ_{occ_name}"
            run_dir = out_dir / tag
            run_dir.mkdir(parents=True, exist_ok=True)

            teacher_a = run_dir / "teacher_ldv_mic"
            teacher_b = run_dir / "teacher_mic_only"
            student_a = run_dir / "student_ldv_mic"
            student_b = run_dir / "student_mic_only"
            ...
            common_teacher = [
                ...
                *speakers,
                ...
                "--occlusion_enable",
                str(int(occ["occlusion_enable"])),
                "--occlusion_target",
                str(occ["occlusion_target"]),
                "--occlusion_kind",
                str(occ["occlusion_kind"]),
                "--occlusion_lowpass_hz",
                str(float(occ["occlusion_lowpass_hz"])),
                "--occlusion_tilt_k",
                str(float(occ["occlusion_tilt_k"])),
                "--occlusion_tilt_pivot_hz",
                str(float(occ["occlusion_tilt_pivot_hz"])),
            ...
            ldv_beats = bool(comp["acceptance"]["overall_pass"])
            claim2_pass = bool(near_fail and ldv_feasible and ldv_beats)
            ...
            summary_rows.append(
                {
                    ...
                    "claim2_checks": {"near_fail": near_fail, "ldv_feasible": ldv_feasible, "ldv_beats_mic_only": ldv_beats},
                    "claim2_pass": claim2_pass,
                }
            )

    any_pass = any(bool(r["claim2_pass"]) for r in summary_rows)
    any_near_fail = any(bool(r["claim2_checks"]["near_fail"]) for r in summary_rows)
    ...
    final = {
        ...
        "rows": summary_rows,
        "final": {
            "claim2_supported_any": bool(any_pass),
            "near_fail_observed_any": bool(any_near_fail),
            "note": (
                "PASS: at least one (SNR, occlusion) satisfied near-fail + feasibility + ablation."
                if any_pass
                else (
                    "FAIL: baseline never reached near-fail in this occlusion grid."
    ...
    lines: list[str] = []
    lines.append("# Claim 2 Verification Report: Occlusion + Noise/Clipping Sweep\n")
    lines.append(f"Generated: {final['generated']}\n")
    lines.append(f"Run dir: `{out_dir}`\n\n")
    lines.append("## Grid\n")
    lines.append("| tag | baseline fail_rate_ref(>5°) | LDV+MIC student fail_rate_ref(>5°) | LDV beats MIC-only | PASS |")
    lines.append("| --- | ---: | ---: | ---: | ---: |")
    for r in summary_rows:
        lines.append(
            f"| {r['tag']} | {r['baseline_fail_rate_ref_gt5deg']:.3f} | {r['ldv_mic_student_fail_rate_ref_gt5deg']:.3f} | {bool(r['claim2_checks']['ldv_beats_mic_only'])} | {bool(r['claim2_pass'])} |"
        )
    lines.append("")
    lines.append("## Final")
    lines.append(f"- claim2_supported_any: **{final['final']['claim2_supported_any']}**")
    lines.append(f"- near_fail_observed_any: {final['final']['near_fail_observed_any']}")
    lines.append(f"- note: {final['final']['note']}")
    (out_dir / "report.md").write_text("\n".join(lines), encoding="utf-8")

    logger.info("Done. Results: %s", out_dir)
    logger.info("Final claim2_supported_any=%s", bool(final["final"]["claim2_supported_any"]))
