#!/usr/bin/env python3
"""
Round 9 lane 1: block7 negative-family target hold.

This lane freezes the round8 winner `b6_family_hold_span2_support2_floor0p08`
and only changes the block7 decision rule. The goal is to keep block7 aligned
to a supported negative family across nearby windows without reopening block6.
"""
...
import round8_block6_positive_family_target_hold_0223 as lane8


WINDOW_SECS = lane8.WINDOW_SECS
WINDOW_OFFSETS_SEC = lane8.WINDOW_OFFSETS_SEC
BASELINE_VARIANT = next(v for v in lane8.VARIANTS if v["name"] == "b6_family_hold_span2_support2_floor0p08")
BLOCK7_CASE_ID = "block7_n08_20"


VARIANTS: list[dict[str, Any]] = [
    {
        "name": "control_round8_ref",
        "neighbor_span": 0,
        "support_required": 99,
        "score_floor": 1.0,
        "vr_tol_ms": 0.0,
    ...
    {
        "name": "b7_neg_hold_span1_support2_floor0p08",
        "neighbor_span": 1,
        "support_required": 2,
        "score_floor": 0.08,
        "vr_tol_ms": 0.20,
    ...
    {
        "name": "b7_neg_hold_span2_support2_floor0p08",
        "neighbor_span": 2,
        "support_required": 2,
        "score_floor": 0.08,
        "vr_tol_ms": 0.22,
    ...
    {
        "name": "b7_neg_hold_span2_support2_floor0p05",
        "neighbor_span": 2,
        "support_required": 2,
        "score_floor": 0.05,
        "vr_tol_ms": 0.25,
...
def choose_negative_candidate(
    ...
    *,
    ...
    for row in scored_pool:
        ...
        if vr_gap > float(vr_tol_ms) or dt_gap > float(abs_dt_tol_ms) or vl_gap > float(vl_tol_ms):
            continue
...
def evaluate_variant_case(
    ...
    *,
    ...
    if variant["name"] == "control_round8_ref" or case.case_id != BLOCK7_CASE_ID:
        return row

    item, scored_pool, _ = lane8.evaluate_pool(case, full_signals, window_sec=window_sec, offset_sec=offset_sec)
    anchor = build_negative_anchor(
        case,
    ...
    candidate = choose_negative_candidate(
        scored_pool,
        anchor,
    ...
    if candidate is None:
        row["negative_anchor"] = anchor
        row["negative_promoted"] = False
        row["promotion_reason"] = None
        return row

    final_best = lane8.annotate_best(candidate, item["reference"], case.case_id)
    ...
    row["negative_anchor"] = anchor
    row["negative_promoted"] = True
    row["promotion_reason"] = "negative_family_target_hold"
    return row


def summarize_setting(rows: list[dict[str, Any]]) -> dict[str, Any]:
    return lane8.summarize_setting(rows)
...
def write_report(out_path: Path, payload: dict[str, Any]) -> None:
    lines = [
        "# Round 9 Block7 Negative-Family Target Hold",
        "",
        f"- Generated: {payload['generated_at']}",
        f"- Data root: `{payload['data_root']}`",
        "- Frozen reference: round8 winner `b6_family_hold_span2_support2_floor0p08`",
        "",
        "## Summary",
        "",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Round 9 block7 negative-family target hold for 0223.")
    parser.add_argument(
        "--data_root",
        type=Path,
    ...
    out_dir = args.out_dir or (
        Path(__file__).resolve().parent.parent / "results" / f"round9_block7_negative_family_target_hold_0223_{timestamp}"
    )
    out_dir.mkdir(parents=True, exist_ok=True)

    full_signals = {case.case_id: lane8.lane2.lane1.load_full_case_signals(case, args.data_root) for case in base.CASES}

    lane6_rows: dict[tuple[str, float, float], dict[str, Any]] = {}
    for case in base.CASES:
        for window_sec in WINDOW_SECS:
            for offset_sec in WINDOW_OFFSETS_SEC:
                lane6_rows[(case.case_id, float(window_sec), float(offset_sec))] = lane8.lane6c.evaluate_variant_case(
                    case,
                    full_signals[case.case_id],
                    window_sec=window_sec,
                    offset_sec=offset_sec,
                    variant=lane8.BASELINE_VARIANT,
                )
    ...
    for case in base.CASES:
        for window_sec in WINDOW_SECS:
            for offset_sec in WINDOW_OFFSETS_SEC:
                baseline_rows[(case.case_id, float(window_sec), float(offset_sec))] = lane8.evaluate_variant_case(
                    case,
                    full_signals[case.case_id],
                    window_sec=window_sec,
                    offset_sec=offset_sec,
