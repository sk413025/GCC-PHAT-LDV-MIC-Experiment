...
DEFAULT_CONFIG = {
    ...
    'gcc_segment_sec': 1.0,
    # To avoid huge full-audio STFTs, analyze a limited time slice
...
def run_stage3_evaluation(
    ...
    *,
    ...
    # Choose analysis center and slice audio to avoid full-file STFTs on long recordings
    min_len = min(len(ldv_signal), len(mic_left_signal), len(mic_right_signal))
    if center_sec is None:
        analysis_center_sample_global = min_len // 2
        analysis_center_sec = analysis_center_sample_global / config['fs']
    else:
        analysis_center_sample_global = int(center_sec * config['fs'])
        analysis_center_sec = float(center_sec)
    ...
    slice_start, slice_end = extract_centered_slice(
        ...
        center_sample=analysis_center_sample_global,
    ...
    logger.info(
        f"Analysis slice: {slice_start/config['fs']:.3f}s–{slice_end/config['fs']:.3f}s "
        f"({(slice_end-slice_start)/config['fs']:.3f}s)"
    )
    ...
    logger.info(f"STFT shapes (slice): LDV={Zxx_ldv.shape}, Left={Zxx_left.shape}")
    ...
    tau_baseline_seg, psr_baseline_seg = gcc_phat_full_analysis(
    ...
    if baseline_method == 'segment':
        tau_ref = tau_baseline_seg
        psr_ref = psr_baseline_seg
        ...
        tau_ref = expected_tdoa
        psr_ref = None

    # Errors vs baseline reference
    error_baseline_seg_vs_ref = float(np.abs(tau_baseline_seg - tau_ref))
    error_raw_vs_ref = float(np.abs(tau_raw - tau_ref))
    error_omp_vs_ref = float(np.abs(tau_omp - tau_ref))

    # Errors vs geometric reference (for logging/debug)
    error_ref_vs_theory = float(np.abs(tau_ref - expected_tdoa))
    error_baseline_seg_vs_theory = float(np.abs(tau_baseline_seg - expected_tdoa))
    error_raw_vs_theory = float(np.abs(tau_raw - expected_tdoa))
    error_omp_vs_theory = float(np.abs(tau_omp - expected_tdoa))

    # Check pass conditions (compare against selected baseline reference)
    error_improved = error_omp_vs_ref < error_raw_vs_ref
    psr_improved = psr_omp > psr_raw
    error_small = error_omp_vs_ref < 0.5e-3  # < 0.5 ms
    ...
    results = {
        ...
        'config': config,
        'analysis': {
            'center_sec': float(analysis_center_sec),
            'slice_start_sec': float(slice_start / config['fs']),
            'slice_end_sec': float(slice_end / config['fs']),
            'omp_start_t': int(start_t),
            'gcc_segment_sec': float(config['gcc_segment_sec']),
        },
        'geometry': {
        ...
        'metrics': {
            'baseline': {
                ...
                'tau_ms': float(tau_ref * 1000),
                'psr_db': None if psr_ref is None else float(psr_ref),
                'error_vs_theory_ms': float(error_ref_vs_theory * 1000),
            },
            'baseline_segment': {
                'pairing': '(MicL, MicR)',
                'tau_ms': float(tau_baseline_seg * 1000),
                'psr_db': float(psr_baseline_seg),
                'error_vs_baseline_ms': float(error_baseline_seg_vs_ref * 1000),
                'error_vs_theory_ms': float(error_baseline_seg_vs_theory * 1000),
            },
            ...
            'raw_ldv': {
                ...
                'error_vs_baseline_ms': float(error_raw_vs_ref * 1000),
                'error_vs_theory_ms': float(error_raw_vs_theory * 1000)
            },
            'omp_ldv': {
                'pairing': '(LDV_as_MicL, MicR)',
                'tau_ms': float(tau_omp * 1000),
                'psr_db': float(psr_omp),
                'error_vs_baseline_ms': float(error_omp_vs_ref * 1000),
                'error_vs_theory_ms': float(error_omp_vs_theory * 1000)
            },
        },
        'pass_conditions': {
    ...
    logger.info("=" * 70)
    logger.info(f"Baseline reference ({baseline_method}): τ = {tau_ref*1000:+.4f} ms, error vs theory = {error_ref_vs_theory*1000:.4f} ms")
    logger.info(f"Geometric expected TDoA (MicL-MicR):   τ = {expected_tdoa*1000:+.4f} ms")
    logger.info(f"GCC-PHAT TDoA ({bp[0]:.0f}-{bp[1]:.0f} Hz bandpass, segment={config['gcc_segment_sec']:.2f}s):")
    logger.info(f"  (MicL, MicR) segment: τ = {tau_baseline_seg*1000:+.4f} ms, PSR = {psr_baseline_seg:.2f} dB, error vs baseline = {error_baseline_seg_vs_ref*1000:.4f} ms")
    logger.info(f"  (Raw_LDV, MicR):      τ = {tau_raw*1000:+.4f} ms, PSR = {psr_raw:.2f} dB, error vs baseline = {error_raw_vs_ref*1000:.4f} ms")
    logger.info(f"  (LDV_as_MicL, MicR):  τ = {tau_omp*1000:+.4f} ms, PSR = {psr_omp:.2f} dB, error vs baseline = {error_omp_vs_ref*1000:.4f} ms")
    logger.info("-" * 70)
    logger.info(f"OMP closer to baseline than raw:    {'✓ PASS' if error_improved else '✗ FAIL'} ({error_omp_vs_ref*1000:.4f} vs {error_raw_vs_ref*1000:.4f} ms)")
    logger.info(f"PSR improved (Raw→OMP):             {'✓ PASS' if psr_improved else '✗ FAIL'} ({psr_raw:.1f} → {psr_omp:.1f} dB)")
    logger.info(f"OMP error vs baseline < 0.5 ms:     {'✓ PASS' if error_small else '✗ FAIL'} ({error_omp_vs_ref*1000:.4f} ms)")
    logger.info("-" * 70)
...
def main():
    ...
    parser.add_argument('--center_sec', type=float, default=None,
                        help='Center time (seconds) for the single evaluation slice. Default: file midpoint.')
