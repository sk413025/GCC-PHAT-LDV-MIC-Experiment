#!/usr/bin/env python3
"""
Phase 4: Final Validation Summary (Chirp References)

Purpose: Aggregate Phase 1-3 results from chirp-reference validation runs
and produce a final report + summary visualization.
...
def load_phase2_summary(run_dir: Path) -> Dict:
    ...
    if window_analysis:
        best_key = min(window_analysis.keys(), key=lambda k: window_analysis[k]['false_peak_rate_guided'])
...
def generate_final_report(results_base: Path) -> Dict:
    print("\n" + "=" * 70)
    print("  PHASE 4: Final Validation Summary (Chirp References)")
    print("=" * 70)

    phase1_run = find_latest_run(results_base / 'chirp_ref_phase1')
    phase2_run = find_latest_run(results_base / 'chirp_ref_phase2')
    phase3_run = find_latest_run(results_base / 'chirp_ref_phase3')
    ...
    if phase1['status'] != 'completed':
        ...
        report['recommendations'].append('Consider alternative signals for evaluation')
    ...
    if phase3['status'] != 'completed':
        ...
        if phase3['new_pass_rate'] < 60:
            report['recommendations'].append('Stage 3 pass rate still low - investigate alignment')
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Phase 4: Final validation summary (chirp references)')
    parser.add_argument('--out_dir', type=Path, default=None,
                        help='Output directory (default: results/chirp_ref_phase4/run_<ts>)')
...
def main() -> int:
    ...
    results_base = script_root / 'results'

    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    if args.out_dir:
        output_dir = args.out_dir
    else:
        output_dir = results_base / 'chirp_ref_phase4' / f'run_{timestamp}'
    output_dir.mkdir(parents=True, exist_ok=True)

    print(f"\n  Output directory: {output_dir}")

    report = generate_final_report(results_base)
    ...
    if report['decision'] == 'VALIDATION_SUCCESS':
        return 0
    if report['decision'] == 'VALIDATION_PARTIAL':
        return 1
    return 2
