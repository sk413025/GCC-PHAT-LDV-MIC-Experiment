...
| | MIC-MIC 全域峰 |err| | MIC-MIC Oracle |err| | LDV S2 |err| | 救援倍數 |
|---|-----------|-----------|-----------|------|
| **block-3(high) +0.4m** | **79.29°** | 0.42° (rank #2) | **7.01°** | **11.3x** |
| **block-2 (低) +0.4m** | **10.70°** | 6.32° (rank #13) | **3.50°** | **3.1x** |

> **遮蔽使 MIC-MIC 全域峰誤差暴增至 10°~79°，加入 LDV-LEFT / LDV-RIGHT 後，誤差被拉回到 3.5°~7°（vs speaker），改善 3~11 倍。**
> MIC-MIC Oracle 表明正確資訊存在（0.42°~6.32°），但全域峰選不到；LDV 的幾何約束幫助自動篩選出更好的結果。

#### 詳細結論列表

| # | 結論 | 證據強度 |
|---|------|---------|
| 1 | **遮蔽癱瘓 MIC-MIC 後，加入 LDV 自動救回：3.1x (block-2) / 11.3x (block-3)** | ✅ 強（§13 兩組驗證） |
| 2 | **MIC-MIC CC(τ) 和 LDV CC(τ) 中都存在正確物理資訊**（各自 Oracle 都很小）| ✅ 強 |
| 2a | — MIC-MIC Oracle: 0.00°~1.44° (vs speaker)，但全域峰被殘響推偏（block-3: rank #2 未被選中） | ✅ 強 |
| 2b | — LDV Oracle: 0.06° (vs v-point theory) → Δτ 解耦公式正確 | ✅ 強（block-4/5 一致） |
| 3 | **遮擋造成額外 73.4° 惡化**（+0.4m: 5.90°→79.29°）→ 板材成功干擾 MIC-MIC | ✅ 強（§10.9） |
| 4 | 無遮擋 baseline 也有 5~9° 誤差 → 殘響環境下 MIC-MIC 本身不可靠 | ✅ 強（§10.9, §11） |
| 5 | LDV 打點上移 → 理論值 peak 排名從 #3~#5 升到 #1（邊界效應確認） | ✅ 強（block-3(high)） |
| 6 | 入射角是 LDV 自動選峰成敗的關鍵因素（+0.4m→#1, +0.8m→#210） | ✅ 強（§11） |
| 7 | 紙板取代鋼板後，MIC-MIC 的失敗模式不變（窄頻鎖 0°） | ✅ 強 |
| 8 | 殘響是根本瓶頸，不是紙板穿透相位差 | ✅ 強（沒板也失敗） |
| 9 | 核心瓶頸已從「解耦是否可行」轉為「如何自動選取正確 peak」 | — 部分解決（§13） |
...
| block-6(high) | -0.4m | **-0.21** | +0.09° | ⚠️ 與上方不同 |

> block-4/5 反推 v\_x = +0.18，block-6 反推 v\_x = -0.21。
> **v 位置不一致** → 可能是不同實驗 session 重新架設時 LDV 打點位置有移動。
> 在各自的 v 位置下 Oracle err 都極小 (0.06°~0.09°) → **解耦公式本身正確，問題在 v 位置校準**。
...
|------|------|
| 數學推導正確性 | ✅ 代數完全正確 |
| 「MIC-MIC 會失敗」的預測 | ✅ 五組 block + 三組 unblock 皆驗證；無擋時已有 1.75~8.86° 誤差，有擋最大 79.29° |
| **「LDV 能救援」的預測** | **✅ 已驗證：MIC-MIC 崩潰時加入 LDV 自動救回 3.1~11.3 倍（§13）** |
| Δτ 公式的準確度 | ✅ Oracle 誤差跨五組穩定在 0.06°~1.18°（公式有效，天花板極高） |
...
| 自動化程度 | ⚠️ 策略2 在小角度可救 3.1~11.3x，大角度自動選峰尚需改進 |
...
| 優先級 | 建議 | 目的 |
|--------|------|------|
| 🔴 高 | **補齊 -0.4m 和 -0.8m 的 block + unblock** | 完成五角度全覆蓋，驗證更多角度的 LDV 救援效果 |
| 🔴 高 | **改進 LDV 自動選峰策略** | 縮小 S2 自動 (3.5°~7°) 與 Oracle (0.06°~1.18°) 的差距 |
| 🔴 高 | **LDV 打點位置優化 + 入射角控制** | 高打點 + 小入射角 → 正確峰排名提升 → 自動成功率提高 |
