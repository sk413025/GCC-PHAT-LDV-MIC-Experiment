...
| -0.8 m | -20.82° | | | | | | ⬜ 待補 |
| -0.4 m | -10.71° | | | | | | ⬜ 待補 |
| +0.0 m | 0.00° | block-5(high) | +0.00° | +4.75° | +0.06° | #89~#195 | ✅ 已分析 (§12) |
...
| 資料集 | 喇叭 | Oracle 反推 v\_x | Oracle err |
|--------|------|-----------------|-----------|
| block-3(high) | +0.4m | (未精確掃描) | +1.18° |
| block-4(high) | +0.8m | **+0.18** | +0.06° |
| block-5(high) | +0.0m | **+0.18** | +0.06° |

> 兩組不同喇叭位置反推出**相同的 v\_x = +0.18** → 空間不變性初步確認。
...
|------|------|
| 數學推導正確性 | ✅ 代數完全正確 |
| 「MIC-MIC 會失敗」的預測 | ✅ 五組 block + 三組 unblock 皆驗證；無擋 baseline 已有 1.75~8.86° 誤差 |
| 「LDV 能解耦」的預測 | ⚠️ 物理延遲資訊存在於 CC(τ) 中，但無法自動提取 |
| Δτ 公式的準確度 | ✅ Oracle 誤差跨五組穩定在 0.06°~1.18°（公式有效） |
| LDV 打點位置影響 | ✅ 上移後 VR 理論值 peak 排名從 #3~#5 升到 #1（邊界效應確認） |
| **入射角影響** | ✅ +0.4m（小角度）→ 自動成功；+0.8m（大角度）→ 穿透能量弱 → 自動失敗 |
| **空間不變性** | ✅ 初步確認：block-4 和 block-5 不同喇叭位置反推相同 v\_x (§12.7) |
| 自動化程度 | ⚠️ 策略2 在小角度可救 3.1~11.3x，其他角度效果有限 |
...
| 優先級 | 建議 | 目的 |
|--------|------|------|
| 🔴 高 | **補齊 -0.4m 和 -0.8m 的 block + unblock** | 完成五角度全覆蓋，驗證負角度是否對稱 |
| 🔴 高 | **LDV 打點位置優化** | §10 證實上移打點 → 理論 peak 排名 #3→#1 |
| 🔴 高 | **控制入射角在合理範圍** | §11 證實大偏角 (+0.8m) 穿透能量弱 → 自動失敗 |
| 🔴 高 | **改進 peak 自動篩選策略** | §13 的策略2 已有 3.1~11.3x 改善，但仍有殘餘誤差 |
| 🔴 高 | 確認 LDV 打點實際座標 | 三組 Oracle 反推 v\_x=+0.18，需雷射尺實測驗證 |
| 🔴 高 | **高打點 + MIC 貼近光點 (<5cm) + Chirp 重做子任務二** | §14 的 90° 驗證受邊界效應限制 |
