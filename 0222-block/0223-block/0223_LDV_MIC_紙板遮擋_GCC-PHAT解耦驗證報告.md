...
```
┌───────────────────────────────────────────────────────────────────────────┐
│                    block         block-2        block-3(high)             │
│ ─────────────────────────────────────────────────────────────────────    │
│ MIC-MIC Fullband   +5.91°        +0.01°         +90.00°                  │
│ MIC-MIC 誤差       -4.80°        -10.70°        +79.29°                  │
│ MIC-MIC 狀態       ⚠️部分成功    ❌ 鎖0°        ❌❌ 完全崩潰            │
│ ─────────────────────────────────────────────────────────────────────    │
│ LDV 策略2 自動     +5.99°        +7.21°         +3.70°                   │
│ LDV 策略2 誤差     -4.72°        -3.50°         -7.01°                   │
│ LDV oracle         +11.47°       +10.28°        +11.89°                  │
│ LDV oracle 誤差    +0.76°        -0.43°         +1.18°                   │
│ ─────────────────────────────────────────────────────────────────────    │
│ 改善倍數(策略2)    ~1x           3.1x           11.3x                    │
│ ─────────────────────────────────────────────────────────────────────    │
│ LDV 打點位置       偏低          偏低           上移 ✅                   │
│ VR 理論peak排名    #3            #5             #1 ⭐                    │
│ ─────────────────────────────────────────────────────────────────────    │
│ 結論: MIC-MIC 越慘 → LDV 救援效果越顯著                                 │
│       LDV 打點上移 → 理論值 peak 排名大幅提升                            │
│       Oracle 誤差穩定在 ≤1.2° → 物理解耦資訊始終存在                    │
└───────────────────────────────────────────────────────────────────────────┘
```
...
| 16 | **無遮擋 baseline 在 +0.8m 就有 +8.86° 誤差 → 殘響環境下 MIC-MIC 本身不可靠** | ✅ 強（§11 unblock-4） |

### 14.2 四組資料集 Oracle 總覽

| 資料集 | 喇叭 | 打點 | MIC-MIC err | LDV S2 err | Oracle err | VR 峰排名 |
|--------|------|------|-------------|------------|------------|----------|
| block | +0.4m | 低 | -4.80° | -4.72° | +0.76° | #3~#5 |
| block-2 | +0.4m | 低 | -10.70° | -3.50° | -0.43° | #3~#5 |
| block-3(high) | +0.4m | **高** | +79.29° | -7.01° | +1.18° | **#1** |
| block-4(high) | **+0.8m** | **高** | -1.44° | +10.02° | **+0.06°** | #210 |
...
|------|------|
| 數學推導正確性 | ✅ 代數完全正確 |
| 「MIC-MIC 會失敗」的預測 | ✅ 實驗驗證（四組：-4.80°, -10.70°, +79.29°；block-4 的 -1.44° 是碰巧，unblock 就有 8.86° 誤差） |
