...
## 12. 子任務二驗證：速度→壓力的 90° 相位偏移 (verification.md)
...
| 8 | **核心瓶頸已從「解耦是否可行」轉為「如何自動選取正確 peak」** | — 部分解決（§10） |
...
| 12 | Oracle 誤差跨三組穩定在 0.43°~1.18° → 物理解耦資訊始終存在 | ✅ 強（三組一致） |
| 13 | 速度→壓力 90° 相移在低頻 (100-300Hz) 和中高頻 (1.5-2kHz) 確認存在 | ⚠️ 部分 PASS（§12） |
| 14 | 600-1500 Hz 偏離 ±90° → 紙板非理想活塞，模態複雜 + 邊界效應 | ✅ 強（分頻段分析） |

### 13.2 theory.md 的有效性評估

| 方面 | 評估 |
|------|------|
| 數學推導正確性 | ✅ 代數完全正確 |
| 「MIC-MIC 會失敗」的預測 | ✅ 實驗驗證（三組皆失敗：-4.80°, -10.70°, +79.29°） |
| 「LDV 能解耦」的預測 | ⚠️ 物理延遲資訊存在於 CC(τ) 中，但無法自動提取 |
| Δτ 公式的準確度 | ✅ 手動選取正確 peak 時，三組 oracle 誤差 0.43°~1.18°（公式有效） |
| LDV 打點位置影響 | ✅ 上移後 VR 理論值 peak 排名從 #3~#5 升到 #1（邊界效應確認） |
| 自動化程度 | ⚠️ 策略2 可救回 3.1~11.3 倍，但仍有 ~3~7° 殘餘誤差 |
| 90° 輻射相移 | ⚠️ 低頻和中高頻確認 ±90°，中頻偏離（板模態 + 邊界效應）（§12） |
| 適用場景 | 理想：無響室或高 SNR；本實驗：需輔助 peak 篩選策略 |
| 本實驗的限制 | 0.1V boy speech + 室內殘響 + LDV 打點偏低 → 需改善實驗條件 |
...
| 🔴 高 | **改進 peak 自動篩選策略** | §11 的策略2 已有 3.1~11.3x 改善，但仍有殘餘誤差；可嘗試多頻段共識、二階篩選 |
| 🔴 高 | **MIC-MIC + LDV 雙系統自動切換** | PSR > 3dB 時信任 MIC-MIC，否則用 LDV 策略2 |
| 🔴 高 | 確認 LDV 打點實際座標 | 縮小 τ 搜索範圍，大幅減少候選組合 |
| 🔴 高 | **高打點 + MIC 貼近光點 (<5cm) + Chirp 重做子任務二** | §12 的 90° 驗證受邊界效應+距離+speech 限制；改善條件後預期可全頻段 PASS |
| 🟡 中 | 分析 0223-unblock（無擋對照） | 確認殘響 vs 紙板的各自貢獻 |
