...
| 9 | **遮蔽癱瘓 MIC-MIC（err -10.70°）後，加入 LDV 自動救回到 err -3.50°** | ✅ 強（block-2 驗證） |
| 10 | 自動方法 vs 手動 oracle（err -0.43°）仍有差距，振幅排名≠正確排名 | ⚠️ 尚需改進 |

### 11.2 theory.md 的有效性評估

| 方面 | 評估 |
|------|------|
| 數學推導正確性 | ✅ 代數完全正確 |
| 「MIC-MIC 會失敗」的預測 | ✅ 實驗驗證（誤差 -4.80°） |
| 「LDV 能解耦」的預測 | ⚠️ 物理延遲資訊存在於 CC(τ) 中，但無法自動提取 |
| Δτ 公式的準確度 | ✅ 手動選取正確 peak 時，誤差僅 0.76°（驗證公式有效） |
| 自動化程度 | ❌ GCC-PHAT 無法自動鎖定正確 peak（20 組合中僅 4 組正確） |
...
| 優先級 | 建議 | 目的 |
|--------|------|------|
| 🔴 高 | **改進 peak 自動篩選策略** | §10 的策略2 已有初步效果（3.1 倍改善），但振幅排名≠正確排名仍是瓶頸。可嘗試：多頻段共識、二階篩選（先策略2 再用頻段穩定性驗證）|
| 🔴 高 | **MIC-MIC + LDV 雙系統自動切換** | §10.5 提出判斷流程：PSR > 3dB 時用 MIC-MIC，否則用 LDV 策略2 |
